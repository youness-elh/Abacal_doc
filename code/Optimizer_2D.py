...
################## Parameters to set ######################
###########################################################
directory = '//isi/w/elh/T_model_2Dopt'#'//isi/w/elh/Optimizer_tool'#'//isi/w/ditt/Optimizer_tool'
...
Q_step1 = np.array([1000.0,1.])
Q_step2 = np.array([10000.0,3.])
nb_col_profiles =80#370 #number of evaluated degrees+3
...
def Gradient_loss(Q1,Q2,profile1,profile2,target,penalisation=1e-30):
	step = Q2 - Q1
	assert step.any() != 0, "Step should be != 0!"
	if type(Q2) != int:
		n = len(Q2)
		grad = np.ndarray(n)
		for i in range(n):
			grad[i] = Loss(Q2,profile2,target,penalisation)-Loss(Q1,profile1,target,penalisation)
			grad[i] /= step[i]
	return grad
...
def Abaqus(Q):
	...
	time.sleep(60)###########make sure extracted data is READY#############
...
error_abs = 1
tol = 1.e-1
max_iter = 24 # (~20 min per cycle)
max_iter_step = 24 # 
step = np.array([19800,0.5,0.5,0.5,0.5])#################tochange#####################
Loss_list = [L1,L2]
...
##################adapt to dimension#######################
###########################################################
dimension = len(Q_step1) #################tochange#####################
step = step[:dimension]
while ((error_abs > tol) and (iter < max_iter)):
	if iter == 1:
		step = np.array([200,0.5,0.5,0.5,0.5])################tochange#####################
		step = step[:dimension]####################
	...
	print('----------------------',gradient)
	Q = np.max([np.array(Q_list[-1]) - step*gradient,np.zeros(dimension)],0)#################tochange#####################
	print('-------------',gradient,step)
	profiles = Abaqus(Q)
	with open(report_file,'r') as f:
		profiles = np.loadtxt(f,delimiter=",")
	...
	while ((count < max_iter_step)  and (delta >=0)):
		...
		step /= 1.3
		Q = np.max([Q_old - step*gradient,np.zeros(dimension)],0)#################tochange#####################
		profiles = Abaqus(Q)
		L_new = Loss(Q,profiles,Targets)
		delta = L_new - Loss_list[-1]
		count +=1
